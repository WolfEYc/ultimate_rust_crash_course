...
trait Plural : ToString {
    fn is_plural(&self) -> bool {
        self.to_string().ends_with("s")
    }
}

impl<T: ToString> Plural for T {}

const SINGULAR_STR: &str = "singular";
const PLURAL_STR: &str = "plural";

fn inspect(s: &String) {
    
    let result = if s.is_plural() { PLURAL_STR } else { SINGULAR_STR };
